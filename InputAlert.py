...
class TextFieldDelegate:
    ...
    def __init__(self, can_move=True):
        ...
        # TODO simplify this into a ternary possibly
        if isinstance(self.can_move, (float, int)) or self.can_move:
            if isinstance(self.can_move, bool):
                self.mb = 100
            else:
                self.mb = can_move
        #print(self.mb)
    ...
    def textfield_did_begin_editing(self, textfield):
        ''' when keyboard pops up, move the InputAlert view up so keyboard does not overlap.
        
        can_move is to be a positive number to move the modal up, negative to move down. if can_move is True, the modal will move by 100px '''
        if not self.can_move:
            return True

        def move_up():
            textfield.superview.y -= self.mb
...
class InputAlert(ui.View):
    '''A ui.View that is added to the root view of a class, as a subview.
    ...
    When editing is begun, the modal will move up '''
    name = 'InputAlert'
    ...
    def input_alert(self,
        ...
        if self.superview:
            dim = ui.View(
                frame=(0, 0, self.superview.width, self.superview.height),
            ...
            inputfield.width = 0.8 * inputbox.width
            inputfield.corner_radius = 0
            inputfield.border_width = 1
            inputfield.action = self.input_action
            inputfield.delegate = TextFieldDelegate(can_move=self.can_move)
...
if __name__ == '__main__':
    ...
    @ui.in_background
    def ia(sender):
        b.bg_color = i.input_alert(
            'enter a color',
            'valid color name then press done.',
            'green',
            can_move=120).lower()
